    ...
    {   
        ...
        if ($settings->allow_multi_currency){
            ...
            if ($request->session()->has('userLocation')) {  
                ...
                try {
                    if ( $user_location && $user_location->ip !== '169.197.85.171' ) {
                        $position = (new Location())->get('169.197.85.171');
                        ...
                        if (!$country){
                            if (in_array( $position->countryName,array_values(Helper::EU()))){
                                ...
                                $rate = ['code' =>  optional($country->rate)->iso_code3, 'rate' => optional($country->rate)->rate,'country' =>$country->country, 'symbol' => $country->symbol ];  
                            } else {
                ...
                try {
                    ...
                    if (null == $country){
                        if (in_array( $position->countryName,array_values(Helper::EU()))){
                            $country = Currency::where('country', 'Europe')->first();
                            $rate = [ 'code' =>  optional($country->rate)->iso_code3, 'rate' => optional($country->rate)->rate,'country' =>$country->name, 'symbol' => $country->symbol ];  
                        } else {
                            $country = Currency::where('country', 'United States')->first();
                            $rate = ['code' =>  optional($country->rate)->iso_code3, 'rate' => optional($country->rate)->rate,'country' =>$country->name, 'symbol' => $country->symbol ];
                        }
