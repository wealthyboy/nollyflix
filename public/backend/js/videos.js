...
$(document).ready(function(){
    $(".search_videos").on('input',function(e){
        var $self = $(this),payLoad = {'title': $self.val()}
        $.ajax({
            type: "GET",
            url: "/admin/related/videos",
            data: payLoad,
        }).done(function(response){
          $("#related_videos").html('').append(response)  
        });
    })
    $(document).on('click','.add_product',function(e){
        e.preventDefault()
        $(this).parentsUntil('tbody').clone().appendTo( ".related_videos" );
        $(this).parentsUntil('tbody').remove()
        $("tbody.related_videos").children('.p').remove()
        $("tbody.related_videos td").children('input.d-none').removeClass('hide')
    })

    $(document).on('click','.remove_related_video',function(e){
        e.preventDefault()
